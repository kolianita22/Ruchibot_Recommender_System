...
recipes = pd.read_csv("cleaned_no_duplicates.csv")
...
def recommend_recipe(user_input, top_n=5):
    ...
    return recipes.iloc[top_indices][['title', 'ingredients_regional','ingredients', 'cuisine', 'diet', 'difficulty', 'score']]
...
def chat():
    ...
    for _, r in results.iterrows():
        reply += f"🍲 <b>{r['title']}</b><b>ingredients:</b>{r['ingredients']}<br><b>ingredients_regional:</b>{r['ingredients_regional']}<br>Cuisine: {r['cuisine']}<br>Diet: {r['diet']}<br>Difficulty: {r['difficulty']}<br><br>"
    return reply
